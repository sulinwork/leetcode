...
public class DisplayTableOfFoodOrdersInARestaurant {
    public static void main(String[] args) {
        ...
        // TO TEST
        List<List<String>> lists = solution.displayTable(new ArrayList<List<String>>() {{
            add(Arrays.asList("Laura", "2", "Bean Burrito"));
            add(Arrays.asList("Jhon", "2", "Beef Burrito"));
            add(Arrays.asList("Melissa", "2", "Soda"));
        }});
        ...
        public List<List<String>> displayTable(List<List<String>> orders) {
            ...
            for (List<String> order : orders) {
                ...
                if (cache.containsKey(number)) {
                    Map<String, Integer> numberCache = cache.get(number);
                    numberCache.put(foodName, numberCache.getOrDefault(foodName, 0) + 1);
                } else cache.put(number, new HashMap<String, Integer>() {{
                    put(foodName, 1);
                }});
            }
            List<String> foodName = title.stream().sorted().collect(Collectors.toList());
            List<List<String>> res = new ArrayList<>();
            res.add(new ArrayList<String>() {{
                add("Table");
                addAll(foodName);
            }});
            List<Integer> cacheKeySortList = cache.keySet().stream().sorted().collect(Collectors.toList());
            for (Integer cacheKey : cacheKeySortList) {
                res.add(new ArrayList<String>() {{
                    add(cacheKey.toString());
                    Map<String, Integer> map = cache.get(cacheKey);
                    for (String foot : foodName) {
                        add(map.getOrDefault(foot, 0).toString());
                    }
                }});
            }
            return res;
