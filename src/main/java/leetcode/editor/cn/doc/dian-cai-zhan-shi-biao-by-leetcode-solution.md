...
- 哈希表 ![\textit{nameSet} ](./p__textit{nameSet}_.png)  保存所有的餐品名称；
- 哈希表 ![\textit{foodsCnt} ](./p__textit{foodsCnt}_.png)  保存桌号及该桌点餐数量，点餐数量也用一个哈希表保存。

遍历订单并保存信息后，从 ![\textit{nameSet} ](./p__textit{nameSet}_.png)  中提取餐品名称，并按字母顺序排列；从 ![\textit{foodsCnt} ](./p__textit{foodsCnt}_.png)  中提取桌号，并按桌号升序排列。然后将餐品名称和桌号分别填入点菜展示表的第一行和第一列。对于表中的餐品数量，我们逐行填入，对于每一行，我们遍历餐品名称，在 ![\textit{foodsCnt} ](./p__textit{foodsCnt}_.png)  中查找对应的点餐数量，然后填入表格中对应位置。
...
- 时间复杂度：![O(T+N\logN+M\logM+MN) ](./p__O_T_+_Nlog_N_+_Mlog_M_+_MN__.png) 。其中 *T* 是数组 ![\textit{orders} ](./p__textit{orders}_.png)  的长度，*N* 是数据表的列数（即餐品的数量），*M* 是数据表的行数（即餐桌的数量）。时间复杂度由以下几个部分组成：
    - 遍历订单并保存信息的时间复杂度为 *O(T)*；
    - 对餐品名称和餐桌编号分别进行排序，时间复杂度分别为 ![O(N\logN) ](./p__O_N_log_N__.png)  和 ![O(M\logM) ](./p__O_M_log_M__.png) ；
